class MovieImage {
    ...
    #validateParams() {
        if (!this.#movie.poster || (!Array.isArray(this.#movie.poster) && typeof this.#movie.poster !== 'string')) {
            throw new Error('Постер должен быть строкой или массивом строк.');
        }

        if (Array.isArray(this.#movie.poster) && this.#movie.poster.length === 0) {
    ...
    #createImageElement() {
        if (Array.isArray(this.#movie.poster) && this.#movie.poster.length > 1) {
            ...
            const posterPath = Array.isArray(this.#movie.poster) ? this.#movie.poster[0] : this.#movie.poster;
    ...
    #createSingleImage(posterPath) {
        ...
        img.src = posterPath || 'http://localhost:3000/posters/no_product_img.png';
