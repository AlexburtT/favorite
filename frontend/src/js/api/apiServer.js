...
class MovieRecords {
    ...
    */
    async fetchFromApi(endpoint = '', options = {}) {
        ...
        try {
            ...
            console.error(`Ошибка при выполнении запроса к API (${url}):`, error.message || error);
            throw error;
        }
    }
    ...
    */
    ...
    async #handleMovieRequest(movieData, method) {
        ...
        movieData.viewed = !!viewed;
        movieData.favorite = !!favorite;
        ...
        if (method === 'POST' && MovieRecords.cache['allMovies']) {
            ...
            const index = MovieRecords.cache['allMovies'].findIndex((movie) => movie.id === id);
            if (index !== -1) {
    ...
    */
    async deleteMovie(id) {
        ...
        if (MovieRecords.cache['allMovies']) {
            MovieRecords.cache['allMovies'] = MovieRecords.cache['allMovies'].filter((movie) => movie.id !== id);
        }
