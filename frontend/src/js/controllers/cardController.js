...
class CardController {
    ...
    async #toggleState(stateKey, button, className) {
        ...
        try {
            ...
            await movieApi.updateMovie(this.#movie.id, { [stateKey]: this.#movie[stateKey] });

            console.log(`Фильм "${this.#movie.name}" помечен как ${this.#movie[stateKey] ? stateKey : 'не ' + stateKey}.`);
        } catch (error) {
    ...
    #updateButtonsState(button, className) {
        ...
        if (button.classList.contains('btn__card--viewed')) {
            ...
            if (viewSpan) {
                button.textContent = this.#movie.viewed ? 'Просмотрено' : 'Не просмотрено';
    ...
    #showFullCardDialog() {
        try {
            ...
            const buttonsContainer = fullCardElement.querySelector('.card__btns--container');
            if (buttonsContainer) {
            ...
            console.error('Ошибка при открытии диалогового окна:', error.message);
        }
    }
    getElement() {
        return this.#element;
